...
def get_all_times():
    display_tracker_all = dict(sorted(tracker_all.items(), key=lambda item: item[1], reverse=True))
    for url, time in display_tracker_all.items():
        display_tracker_all[url] = _convert_time(time)
    return display_tracker_all

def get_week_times():
    display_tracker_week = dict(sorted(tracker_week.items(), key=lambda item: item[1], reverse=True))
    for url, time in display_tracker_week.items():
        display_tracker_week[url] = _convert_time(time)
    return display_tracker_week

def get_day_times():
    display_tracker_day = dict(sorted(tracker_day.items(), key=lambda item: item[1], reverse=True))
    for url, time in display_tracker_day.items():
